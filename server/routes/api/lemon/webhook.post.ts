...
export default defineEventHandler(async (event) => {
  ...
  console.log(`[Webhook] Called | body: ${rawBody.length} bytes | sig: ${signature ? 'present' : 'missing'}`);
  ...
  if (eventName === 'order_created' || eventName === 'subscription_created') {
    ...
    const tier = variantId === creatorVariant ? 'creator' : variantId === proVariant ? 'pro' : 'free';
    const amount = tier === 'pro' ? 25 : tier === 'creator' ? 10 : 0;

    console.log(`[Webhook] Recording: email=${userEmail} tier=${tier} variant=${variantId} ref=${referrer} amount=${amount}`);
