...
export interface DropoffMarker {
  timestamp: number;
  reason: "Low novelty" | "Cut gap" | "Audio dip" | "Pacing slump" | "Hook fade" | "Energy plateau" | "Micro drop";
...
export interface DropoffPrediction {
  ...
  retentionAt: { five: number; fifteen: number; thirty: number; sixty: number; midpoint: number; end: number };
...
export function predictDropoffs(meta: DropoffMeta): DropoffPrediction {
  ...
  const rng = seeded(Math.round(durationSec * 100 + hookScore * 10 + cutDensity * 50));
  ...
  if (hookScore < hookThreshold) {
    const hookTs = isShort ? +(rng() * 1.5 + 0.3).toFixed(1) : +(rng() * 2.5 + 0.5).toFixed(1);
    ...
    markers.push({
      ...
      fixHint: pick(HINTS["Low novelty"]),
    });
  }

  if (hookScore >= 55 && hookScore <= 85) {
    const fadeTs = isShort ? +(rng() * 1 + 2).toFixed(1) : +(rng() * 3 + 4).toFixed(1);
    ...
    markers.push({
      ...
      fixHint: pick(HINTS["Hook fade"]),
    });
  }

  if (durationSec > 20 && cutDensity < 0.65) {
    const pct = isShort ? 0.3 + rng() * 0.15 : 0.25 + rng() * 0.2;
    const gapTs = +(durationSec * pct).toFixed(1);
    ...
    markers.push({
      ...
      fixHint: pick(HINTS["Cut gap"]),
    });
  }

  if (audioEnergy < 0.75) {
    const dipPct = isShort ? 0.4 + rng() * 0.3 : 0.45 + rng() * 0.25;
    ...
    markers.push({
      ...
      fixHint: pick(HINTS["Audio dip"]),
    });
  }

  if (durationSec > 30 && cutDensity < 0.7 && audioEnergy < 0.8) {
    const pltPct = isShort ? 0.55 + rng() * 0.2 : 0.5 + rng() * 0.25;
    ...
    markers.push({
      ...
      fixHint: pick(HINTS["Energy plateau"]),
    });
  }

  const endPct = isShort ? 0.7 + rng() * 0.12 : 0.72 + rng() * 0.15;
  const slumpTs = +(durationSec * endPct).toFixed(1);
  markers.push({
    timestamp: slumpTs,
    reason: "Pacing slump",
    severity: "low",
    confidence: Math.round(70 + rng() * 15),
    fixHint: pick(HINTS["Pacing slump"]),
  });

  markers.sort((a, b) => a.timestamp - b.timestamp);

  // Micro-markers — subtle noise markers
  const microCount = isShort ? Math.floor(2 + rng() * 3) : Math.floor(3 + rng() * 5);
  const microMarkers: { timestamp: number; severity: DropoffSeverity }[] = [];
  for (let i = 0; i < microCount; i++) {
    const mt = +(rng() * (durationSec - 1) + 0.5).toFixed(1);
    if (markers.some((m) => Math.abs(m.timestamp - mt) < 3)) continue;
    microMarkers.push({
      timestamp: mt,
      severity: rng() < 0.3 ? "medium" : "low",
    });
  }
  microMarkers.sort((a, b) => a.timestamp - b.timestamp);
  ...
  const lowerCurve = curve.map((p) => ({
    t: p.t,
    v: +clip(p.v - bandWidth + rng() * 2, 18, 100).toFixed(1),
  }));
  ...
  // Niche comparison — how this curve compares to typical
  ...
  try { if (typeof sessionStorage !== "undefined") sessionStorage.setItem(cacheKey, JSON.stringify(result)); } catch {}
