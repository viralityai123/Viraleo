...
const SAME_DAY_OFFERS = `\nDRAFT-C RULES (the close draft — only for hot leads):\n- Web/design/branding/landing leads: use the DIRECT-CLOSE pitch verbatim — "${THREADS_CONFIG.sameDayOffer}" — adapt only the delivery promise. NEVER push meetings/calls in draftC; quote and close in the chat. NO hard prices — the fixed price comes with the proposal.\n- Video editing leads: "${THREADS_CONFIG.sameDayOfferVideo}"\n- Designers/agencies needing clients (lead-service): "${THREADS_CONFIG.leadServiceOffer}" — sell the lead feed, not design work.`;
...
- draftC (ONLY when intentScore >= 85 or their post is a clear job/hire request): the DIRECT-CLOSE pitch from the DRAFT-C RULES block, adapted to THEIR project. Proposal tonight + fixed price + approve-before-paying. NO meetings/calls in text — close in the chat. For every other lead, return draftC as an EMPTY string.
