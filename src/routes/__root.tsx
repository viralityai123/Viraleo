...
import { UserStateProvider, useUserState } from "@/lib/user-state";
    ...
    <div className="flex min-h-screen items-center justify-center bg-surface px-4">
      <div className="max-w-md text-center">
        ...
        <div className="mt-8 flex flex-col gap-2">
          ...
          <a href="/" className="text-xs text-ink-soft hover:text-ink underline underline-offset-2">
            Go home
          </a>
        </div>
      </div>
  ...
  head: () => ({
    meta: [
      ...
      { title: "Viraleo — YouTube Channel Intelligence, decoded" },
      {
        name: "description",
        content:
          "Analyze any YouTube channel and unlock viral patterns, hooks, thumbnails, and structure. AI-powered thumbnail testing, niche ranking, and shadowban detection.",
      },
      ...
      { name: "twitter:title", content: "Viraleo — YouTube Channel Intelligence, decoded" },
      {
        ...
        content:
          "Analyze any YouTube channel and unlock viral patterns, hooks, thumbnails, and structure.",
      },
      ...
      <head>
        ...
        <script type="application/ld+json" suppressHydrationWarning>
          {JSON.stringify({
            ...
            description:
              "AI-powered YouTube channel intelligence — thumbnail testing, niche ranking, shadowban detection, and pre-upload audit.",
...
function useSession() {
  ...
  useEffect(() => {
    ...
    setSession(s);
    if (s?.email) setActivityEmail(s.email);
    setLoaded(true);
  }, []);
  const signOut = () => {
...
function RecentActivityList({ userEmail }: { userEmail: string | null }) {
  const [items, setItems] = useState<ActivityEntry[]>([]);
  const pathname = useRouterState({ select: (s) => s.location.pathname });

  useEffect(() => {
    if (userEmail) {
      setActivityEmail(userEmail);
  ...
  return (
    <div className="mt-1" onMouseEnter={() => setItems(getRecentActivities(5))}>
      <div className="beautiful-sidebar-item !pb-1 !pt-2 !cursor-default hover:!bg-transparent hover:!shadow-none hover:!translate-y-0">
        ...
        <span className="label text-[10px] font-bold uppercase tracking-widest text-ink-soft">
          Recent
        </span>
      </div>
      {items.map((a) => (
      ...
      <Link to="/history" className="beautiful-sidebar-item !py-1.5">
...
function RootSidebar({
  ...
  return (
    <aside className="beautiful-sidebar">
      <div className="beautiful-sidebar-title">Intelligence Suite</div>
      ...
      <RecentActivityList userEmail={session?.email ?? null} />
      ...
      {session ? (
        <>
          ...
          <Link
            ...
            className="beautiful-sidebar-item border-t border-hairline pt-3 mt-1 !text-ink-soft hover:!text-red-600 hover:!bg-red-50 hover:!border-red-100 hover:!shadow-none hover:!translate-y-0"
          >
            <svg
              width="20"
              height="20"
...
function RootComponent() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <UserStateProvider email={session?.email ?? null}>{content}</UserStateProvider>
    </QueryClientProvider>
  );
}
