...
const fetchSubscription = createServerFn({ method: "POST" }).handler(async () => {
  ...
  try {
    const res = await fetch(
      `https://api.lemonsqueezy.com/v1/subscriptions?filter[store_id]=${storeId}&page[size]=10`,
      {
  ...
  .handler(async ({ data }) => {
    ...
    try {
      const res = await fetch(`https://api.lemonsqueezy.com/v1/subscriptions/${data.id}`, {
        ...
        body: JSON.stringify({
          data: { type: "subscriptions", id: data.id, attributes: { cancelled: true } },
        }),
      });
      return { ok: res.ok };
    } catch {
  ...
  .handler(async ({ data }) => {
    ...
    try {
      const res = await fetch(`https://api.lemonsqueezy.com/v1/subscriptions/${data.id}`, {
        ...
        body: JSON.stringify({
          data: { type: "subscriptions", id: data.id, attributes: { cancelled: false } },
        }),
      });
      return { ok: res.ok };
    } catch {
...
export const Route = createFileRoute("/billing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Manage your Viraleo subscription, view invoices, and update billing information.",
      },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${colors[status] || "bg-ink-soft/10 text-ink-soft"}`}
    >
      <Icon size={12} /> {status}
    </span>
  );
...
function BillingPage() {
  ...
  const { label: planLabel, price: planPrice } = usePlanDisplay();
  ...
  async function handleCancel(id: string) {
    ...
    if (result.ok) {
      setSubs(null);
      fetchSubscription().then(setSubs);
    }
    setLoading(false);
  }
    ...
    <div className="min-h-screen bg-white text-ink font-text">
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="flex items-center gap-4 mb-8">
          <Link to="/account" className="p-2 rounded-xl hover:bg-surface-2 transition">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-white border border-emerald-200 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[13px] font-bold text-emerald-700">{planLabel}</span>
              </div>
              <p className="text-[13px] text-ink-soft">{planPrice}</p>
            ...
            {attrs && (
              <div className="bg-white border border-hairline rounded-2xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-[13px]">
                  <div>
                    <span className="text-ink-soft block">Plan</span>
                    <span className="font-semibold text-ink">
                      {attrs.productName} — {attrs.variantName}
                    </span>
                  </div>
                  <div>
                    <span className="text-ink-soft block">Status</span>
                    <StatusBadge status={attrs.status} />
                  </div>
                  {attrs.renewsAt && (
                    <div>
                      <span className="text-ink-soft block">Renews</span>
                      <span className="font-semibold text-ink">
                        {new Date(attrs.renewsAt).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                  {attrs.endsAt && (
                    <div>
                      <span className="text-ink-soft block">Ends</span>
                      <span className="font-semibold text-ink">
                        {new Date(attrs.endsAt).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                  <div>
                    <span className="text-ink-soft block">Created</span>
                    <span className="font-semibold text-ink">
                ...
                <a
                  href={attrs.urls.customerPortal}
                  target="_blank"
