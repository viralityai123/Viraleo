...
import { letterGradeTen, normalizeThumbnailScores } from "@/lib/score-scale";
  ...
  head: () => ({
    meta: [
      { title: "Thumbnail Simulator — Viraleo" },
      {
        name: "description",
        content:
          "Test YouTube thumbnails with AI. Score your thumbnail design, compare variants, and predict CTR before you upload.",
      },
      { property: "og:title", content: "Thumbnail Simulator — Viraleo" },
      {
        property: "og:description",
        content: "Test YouTube thumbnails with AI. Score designs, compare variants, predict CTR.",
      },
...
export const analyzeThumbnailServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // Auth only — credit already consumed by consumeCredit() before this is called
...
const MOCK_LONG = [
  {
    ...
    views: "45M",
    time: "2 days ago",
    img: "https://images.unsplash.com/photo-1593640408182-31c70c8268f5?w=640&h=360&fit=crop",
    color: "bg-blue-600",
  },
  {
    id: 2,
    title: "100 Days Building a Modern Cabin",
    channel: "Cabin Life",
    views: "2.1M",
    time: "1 week ago",
    img: "https://images.unsplash.com/photo-1449844908441-8829872d2607?w=640&h=360&fit=crop",
  ...
  },
  {
    ...
    views: "850K",
    time: "3 days ago",
    img: "https://images.unsplash.com/photo-1550009158-9ebf69173e03?w=640&h=360&fit=crop",
    color: "bg-purple-600",
  },
  {
    id: 4,
    title: "How to perfectly roast a chicken",
    channel: "Chef's Kitchen",
    views: "1.2M",
    time: "1 month ago",
    img: "https://images.unsplash.com/photo-1598514982205-f36b96d1e8d4?w=640&h=360&fit=crop",
    color: "bg-red-600",
  },
  {
    id: 5,
    ...
    views: "450K",
    time: "2 weeks ago",
    img: "https://images.unsplash.com/photo-1593640495253-23196b27a87f?w=640&h=360&fit=crop",
    color: "bg-indigo-600",
  },
];
...
const MOCK_SHORT = [
  {
    id: 1,
    title: "Satisfying pressure washing 💦",
    views: "12M",
    img: "https://images.unsplash.com/photo-1527264935190-1401c51b5bbc?w=360&h=640&fit=crop",
  },
  {
    id: 2,
    title: "I can't believe this worked! 🤯",
    views: "8.5M",
    img: "https://images.unsplash.com/photo-1616423640778-28d1b53229bd?w=360&h=640&fit=crop",
  },
  {
    id: 3,
    title: "Wait for the plot twist...",
    views: "3.2M",
    img: "https://images.unsplash.com/photo-1518791841217-8f162f1e1131?w=360&h=640&fit=crop",
  },
  {
    id: 4,
    title: "Coolest gadget ever? 📱",
    views: "5.1M",
    img: "https://images.unsplash.com/photo-1546868871-7041f2a55e12?w=360&h=640&fit=crop",
  },
];
...
function computeDeterministicScore(features: ThumbnailFeatures): number {
  ...
  return Math.min(
    ...
    Math.max(
      0,
      +((m.visualContrast + m.textReadability + m.topicRelevance + m.clickPsychology) / 4).toFixed(
        1,
      ),
    ),
  );
...
function ThumbnailTestPage() {
  const { hasCredits, consumeCredit, loading: creditsLoading } = useUserState();
  ...
  useEffect(() => {
    if (activityIdParam) {
      ...
      if (cached) {
        const { _meta, ...report } = cached as typeof cached & Record<string, unknown>;
        setAiData(report);
        if (_meta?.thumbBase64) {
          setBase64(_meta.thumbBase64);
          setThumbUrl(_meta.thumbBase64);
        }
        if (_meta?.title) setTitle(_meta.title);
        if (_meta?.context) setContext(_meta.context);
        if (_meta?.channelName) setChannelName(_meta.channelName);
        if (_meta?.format) setFormat(_meta.format);
        setPhase("results");
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    extractThumbnailFeatures(base64, t, format === "short")
      .then((features) => {
  ...
  const runAnalysis = async () => {
    if (!base64 || !title || !context) {
      toast.error(
        "Please enter a title and video context so the AI can rank your thumbnail's relevance.",
      );
      return;
    }
    ...
    try {
      ...
      if (llmResult) {
        llmResult = normalizeThumbnailScores(llmResult);
      }

      const llmOverall = llmResult?.metrics?.length
        ? +(
            llmResult.metrics.reduce((s: number, m: { score: number }) => s + m.score, 0) /
            llmResult.metrics.length
          ).toFixed(1)
        : thumbFeatures
          ? computeDeterministicScore(thumbFeatures)
          : (llmResult?.overallScore ?? 5);
      const result = {
        ...llmResult,
        overallScore: llmOverall,
      ...
      saveResult(entry.id, {
        ...result,
        _meta: { thumbBase64: base64, title, context, channelName, format },
      });
      ...
      if (msg.includes("UNAUTHORIZED")) {
        toast.error("You must be signed in to run an analysis.");
      } else if (msg.includes("OUT_OF_CREDITS")) {
        toast.error("You're out of credits for this month. Upgrade your plan to continue.");
      ...
      clearInterval(interval);
      // Only advance to results if the analysis actually succeeded.
      // Without this check, finally would override setPhase("setup") from catch.
  ...
  const acceptB = useCallback(
    (f: File) => {
      ...
      reader.onloadend = () => {
        ...
        extractThumbnailFeatures(b64, title || "Untitled", format === "short")
          .then(async (features) => {
            ...
            const bOverall = llmResult?.metrics?.length
              ? +(
                  llmResult.metrics.reduce((s: number, m: any) => s + m.score, 0) /
                  llmResult.metrics.length
                ).toFixed(1)
              : score;
            setAiDataB({
  ...
  function onFeedback(vote: "positive" | "negative") {
    ...
    if (thumbFeatures && aiData) {
      const record = collectTrainingData(
        undefined,
        {
          markers: [],
          microMarkers: [],
  ...
  const renderLongItem = (mock: any, isUser: boolean) => {
    ...
    const cardBorder = isUser
      ? "border-[2px] border-good shadow-[0_0_20px_rgba(61,220,107,0.15)] ring-2 ring-good/20 z-10 relative"
      : isDark
        ? "bg-[#222]"
        : "bg-gray-200";
      ...
      return (
        <div key={isUser ? "user" : mock.id} className="flex flex-col cursor-pointer group">
          ...
          <div className="flex gap-3 mt-3">
            <div
              className={`w-9 h-9 rounded-full shrink-0 flex items-center justify-center font-bold text-white ${isUser ? "bg-good text-ink" : mock.color}`}
            >
              {isUser ? (channelName ? channelName.charAt(0).toUpperCase() : "Y") : ""}
            </div>
            <div className="flex flex-col">
              <h3
                className={`font-semibold text-[15px] leading-tight line-clamp-2 mb-1 ${isDark ? "text-[#f1f1f1]" : "text-[#0f0f0f]"}`}
              >
                {finalTitle}
              </h3>
              <div
                className={`text-[13px] leading-tight ${isDark ? "text-[#aaaaaa]" : "text-[#606060]"}`}
              >
                {finalChannel}
                <br />
                {finalViews} • {finalTime}
              </div>
            </div>
          </div>
        </div>
    ...
    if (layoutMode === "sidebar") {
      return (
        <div
          key={isUser ? "user" : mock.id}
          className="flex gap-2 cursor-pointer group items-start"
        >
          <div
            className={`relative w-[168px] shrink-0 aspect-video rounded-xl overflow-hidden ${cardBorder}`}
          >
            <img
              src={finalImg!}
              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
          ...
          <div className="flex flex-col py-0.5">
            <h3
              className={`font-semibold text-[14px] leading-tight line-clamp-2 mb-1 ${isDark ? "text-[#f1f1f1]" : "text-[#0f0f0f]"}`}
            >
              {finalTitle}
            </h3>
            <div
              className={`text-[12px] leading-tight ${isDark ? "text-[#aaaaaa]" : "text-[#606060]"}`}
            >
              {finalChannel}
              <br />
              {finalViews} • {finalTime}
    ...
    if (layoutMode === "mobile") {
      return (
        <div key={isUser ? "user" : mock.id} className="flex flex-col cursor-pointer group w-full">
          <div
            className={`relative w-full aspect-video ${cardBorder} ${!isUser ? (isDark ? "border-none rounded-none border-b border-[#303030]" : "border-none rounded-none border-b border-gray-200") : ""}`}
          >
          ...
          <div className="flex gap-3 p-4">
            <div
              className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center font-bold text-white ${isUser ? "bg-good text-ink" : mock.color}`}
            >
              {isUser ? (channelName ? channelName.charAt(0).toUpperCase() : "Y") : ""}
            </div>
            <div className="flex flex-col justify-center">
              <h3
                className={`font-semibold text-[16px] leading-tight line-clamp-2 mb-1 ${isDark ? "text-[#f1f1f1]" : "text-[#0f0f0f]"}`}
              >
                {finalTitle}
              </h3>
              <div
                className={`text-[13px] leading-tight ${isDark ? "text-[#aaaaaa]" : "text-[#606060]"}`}
              >
                {finalChannel} • {finalViews} • {finalTime}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-surface text-ink font-text relative">
      ...
      {(phase === "setup" || phase === "analyzing" || phase === "results") && (
        ...
        <main className="relative mx-auto max-w-3xl px-6 pt-24 pb-24 z-10">
          ...
          >
            ...
            <p className="mt-3 text-[15px] text-ink-soft max-w-md mx-auto">
              Drop your image to instantly simulate live feed layouts and run a full AI Vision scan
              against your video's topic.
            </p>
          </motion.div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="mt-1.5 text-[13px] text-ink-soft">
                    JPG, PNG, WebP · 16:9 or 9:16 automatically detected
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
        ...
        <main className="relative mx-auto max-w-3xl px-6 pt-12 pb-24 z-10">
          ...
          >
            ...
            <p className="text-[15px] text-ink-soft">
              Tell the AI what the video is about so it can rank visual relevance.
            </p>
          </motion.div>
          <div className="bg-surface border border-hairline rounded-[28px] p-8 shadow-xl flex flex-col md:flex-row gap-8">
            <div className="w-full md:w-1/2 flex flex-col gap-2">
              <div className="text-[13px] uppercase tracking-widest text-ink-soft font-semibold mb-1">
                Detected Upload
              </div>
              <div
                className={`relative w-full rounded-2xl overflow-hidden bg-surface-2 border border-hairline ${format === "short" ? "aspect-[9/16] max-w-[200px] mx-auto" : "aspect-video"}`}
              >
                <img
                  src={thumbUrl!}
            ...
            <div className="w-full md:w-1/2 flex flex-col gap-5 justify-center">
              <div>
                <label className="text-[13px] font-semibold text-ink-soft mb-2 block">
                  Video Title
                </label>
                <input
                  type="text"
              ...
              <button
                onClick={runAnalysis}
                className="w-full bg-ink text-surface font-semibold py-3.5 rounded-xl hover:opacity-90 transition shadow-lg mt-2"
              >
        ...
        <main className="relative mx-auto max-w-sm px-6 pt-32 pb-24 z-10 text-center cursor-none select-none">
          <div className="mx-auto size-20 rounded-3xl bg-good/15 border border-good/30 flex items-center justify-center mb-6">
            <motion.div
              ...
              className="size-8 rounded-full border-2 border-good/30 border-t-good"
            />
          </div>
          ...
          <p className="text-[12px] text-ink-soft/60 mb-6">
            Multimodal AI is extracting objects, reading text, and checking safe zones.
          </p>
        ...
        <main className="mx-auto max-w-[1400px] px-4 sm:px-6 pt-8 pb-24 relative z-10">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
            ...
            <div className="xl:col-span-8 flex flex-col order-2 xl:order-1">
              <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-4">
                <div>
                  ...
                  <p className="text-ink-soft text-[15px] mt-1">
                    Exactly how your {format === "short" ? "Short" : "video"} will look next to
                    competitors.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  {format === "long" && (
                    <div className="flex bg-surface-2 border border-hairline rounded-lg p-1 shrink-0 hidden sm:flex">
                      <button
                        ...
                        className={`px-3 py-1.5 text-[12px] font-semibold rounded-md flex items-center gap-1.5 transition ${layoutMode === "grid" ? "bg-surface text-ink shadow-[0_2px_8px_rgba(0,0,0,0.06)]" : "text-ink-soft hover:text-ink"} `}
                      >
                        <LayoutGrid size={14} /> Grid
                      </button>
                      <button
                        onClick={() => setLayoutMode("sidebar")}
                        className={`px-3 py-1.5 text-[12px] font-semibold rounded-md flex items-center gap-1.5 transition ${layoutMode === "sidebar" ? "bg-surface text-ink shadow-[0_2px_8px_rgba(0,0,0,0.06)]" : "text-ink-soft hover:text-ink"} `}
                      >
                        <ListMinus size={14} /> Sidebar
                      </button>
                      <button
                        onClick={() => setLayoutMode("mobile")}
                        className={`px-3 py-1.5 text-[12px] font-semibold rounded-md flex items-center gap-1.5 transition ${layoutMode === "mobile" ? "bg-surface text-ink shadow-[0_2px_8px_rgba(0,0,0,0.06)]" : "text-ink-soft hover:text-ink"} `}
                      >
                  ...
                  <div className="flex bg-surface-2 border border-hairline rounded-lg p-1 shrink-0">
                    <button
                      ...
                      className={`px-3 py-1.5 text-[13px] font-semibold rounded-md flex items-center gap-2 transition ${theme === "light" ? "bg-surface text-ink shadow-[0_2px_8px_rgba(0,0,0,0.06)]" : "text-ink-soft hover:text-ink"}`}
                    >
                      <Sun size={14} />
                    </button>
                    <button
                      onClick={() => setTheme("dark")}
                      className={`px-3 py-1.5 text-[13px] font-semibold rounded-md flex items-center gap-2 transition ${theme === "dark" ? "bg-ink text-surface shadow-[0_2px_8px_rgba(0,0,0,0.15)]" : "text-ink-soft hover:text-ink"}`}
                    >
                      <Moon size={14} />
                    </button>
                  </div>
              ...
              <div
                className={`w-full rounded-[24px] overflow-hidden border ${isDark ? "border-[#303030] bg-[#0F0F0F]" : "border-hairline bg-[#F9F9F9]"} transition-colors duration-300 pb-16 shadow-2xl relative`}
              >
                {/* Header Mock */}
                <div
                  className={`flex items-center justify-between px-6 py-4 ${isDark ? "text-white border-b border-[#303030]" : "text-black border-b border-gray-200"} ${layoutMode === "mobile" && format === "long" ? "hidden" : ""}`}
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex items-center gap-1 font-bold text-[22px] tracking-tighter">
                      <div className="w-8 h-5.5 bg-red-600 rounded-[6px] flex items-center justify-center">
                  ...
                  <div className="hidden md:flex flex-1 max-w-xl mx-12">
                    <div
                      className={`flex items-center w-full rounded-full border ${isDark ? "border-[#303030] bg-[#121212]" : "border-gray-300 bg-white"} overflow-hidden h-10`}
                    >
                      <div
                        className={`flex-1 px-5 font-normal ${isDark ? "text-gray-400" : "text-gray-500"}`}
                      >
                        Search
                      </div>
                      <div
                        className={`px-5 h-full flex items-center justify-center border-l ${isDark ? "border-[#303030] bg-[#222222]" : "border-gray-300 bg-gray-50"}`}
                      >
                        <SearchIcon
                          size={18}
                  ...
                  >
                    {layoutMode === "mobile" && (
                      <div className="flex items-center justify-between px-4 py-4 text-white border-b border-[#303030]">
                        <div className="flex items-center gap-1 font-bold text-[20px] tracking-tighter">
                          <div className="w-7 h-5 bg-red-600 rounded-[6px] flex items-center justify-center">
                    ...
                    {longMocksSequence.map((m: any, i) =>
                      renderLongItem(m.isUser ? {} : m, !!m.isUser),
                    )}
                  </div>
                )}
                ...
                {format === "short" && (
                  <div className="p-6 md:p-8">
                    <div
                      className={`font-bold text-xl mb-6 flex items-center gap-2 ${isDark ? "text-white" : "text-black"}`}
                    >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                      ...
                      <div className="flex flex-col cursor-pointer group">
                        <div
                          className={`relative aspect-[9/16] rounded-xl overflow-hidden border-[2px] border-good shadow-[0_0_20px_rgba(61,220,107,0.15)] ring-2 ring-good/20 z-10`}
                        >
                          <img
                            src={thumbUrl || ""}
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                          ...
                          <div className="absolute bottom-3 left-3 right-3 text-white">
                            <div
                              className="font-semibold text-[14px] line-clamp-2 mb-1 leading-[1.2] text-white"
                      ...
                      {MOCK_SHORT.map((mock, idx) => (
                        <div
                          key={mock.id}
                          className={`flex flex-col cursor-pointer group ${idx > 2 ? "hidden lg:flex" : ""}`}
                        >
                          <div
                            className={`relative aspect-[9/16] rounded-xl overflow-hidden ${isDark ? "bg-[#222]" : "bg-gray-200"}`}
                          >
                            <img
                              src={mock.img}
                              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
              ...
              <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
                    ...
                    <div>
                      <div className="flex justify-between text-[13px] font-semibold mb-1.5">
                        ...
                        <span className="text-good">
                          {(thumbFeatures ? thumbFeatures.predictedCtr * 100 : 4.5).toFixed(1)}%
                        </span>
                      </div>
                      <div className="h-2 w-full bg-surface-2 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (thumbFeatures ? thumbFeatures.predictedCtr / 0.12 : 0.35) * 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                ...
                {!thumbUrlB ? (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-ink text-[16px] mb-2 z-10">
                      Run A/B Split Test
                    </h3>
                    <p className="text-[13px] text-ink-soft mb-5 max-w-[240px] z-10">
                      Upload an alternative variation of your thumbnail to immediately compare
                      visual scores side-by-side.
                    </p>
                  ...
                  <div className="bg-surface border border-hairline rounded-[24px] p-6 shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        ...
                        className="text-[11px] font-bold text-ink-soft hover:text-red-500 uppercase"
                      >
                        Clear B
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="flex flex-col gap-2">
                        <div
                          className={`relative aspect-video rounded-lg overflow-hidden border-2 ${!aiDataB || aiData.overallScore >= aiDataB.overallScore ? "border-good shadow-[0_0_15px_rgba(61,220,107,0.15)]" : "border-hairline"}`}
                        >
                        ...
                        <div className="text-center font-bold text-[18px]">
                          {aiData.overallScore}/10
                        </div>
                      </div>

                      <div className="flex flex-col gap-2">
                        <div
                          className={`relative aspect-video rounded-lg overflow-hidden border-2 ${aiDataB && aiDataB.overallScore > aiData.overallScore ? "border-good shadow-[0_0_15px_rgba(61,220,107,0.15)]" : "border-hairline"}`}
                        >
                        ...
                        <div className="text-center font-bold text-[18px] text-ink">
                          {isAnalyzingB ? "..." : aiDataB ? `${aiDataB.overallScore}/10` : "-"}
                        </div>
                      </div>
                    </div>
            ...
            <div className="xl:col-span-4 flex flex-col order-1 xl:order-2">
              <div className="mb-6">
                <div className="text-[12px] uppercase tracking-[0.14em] text-ink-soft font-medium">
                  Auto-detected: {format === "short" ? "9:16 Short" : "16:9 Long-form"}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-surface border border-hairline rounded-[24px] p-6 shadow-sm mt-4">
                  <div className="text-[13px] font-semibold text-ink-soft mb-2 block uppercase tracking-wider">
                    Video Title Tested
                  </div>
                  ...
                  <div className="text-[14px] text-ink-soft leading-relaxed bg-surface-2 p-3 rounded-lg">
                    {context}
                  </div>
                </div>
              </div>
          ...
          <UpgradeBanner
            title="Get more thumbnail audits"
            description="Upgrade to Creator for more monthly analyses, A/B comparison, and priority AI vision processing."
          />
        </main>
      )}
    </div>
