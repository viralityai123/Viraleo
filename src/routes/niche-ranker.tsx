      ...
      (record as any).llmMetrics = [
        {
          ...
          copy: data.metrics.saturation.insight,
        },
        {
          label: "Trend",
        ...
        },
        {
          ...
          copy: data.metrics.breakthroughDifficulty.insight,
        },
      ];
            ...
            <div className="text-center mb-12">
              ...
              <h1 className="font-display text-[52px] md:text-[64px] font-bold leading-[1.01] tracking-[-0.03em] text-ink mb-4">
                ...
                <span className="relative inline-block mt-2">
                  worth it?
                  <svg
                    className="absolute w-[110%] h-6 -bottom-2 -left-[5%] z-[-1] pointer-events-none"
                    viewBox="0 0 100 20"
              ...
              <p className="text-[16px] text-ink-soft max-w-md mx-auto">
                Enter any YouTube niche idea or paste a competitor's video link. Our AI will extract
                the niche and give you a brutally honest viability score.
              </p>
            </div>
        ...
        {phase === "loading" && (
          ...
          >
            ...
            <div className="text-center">
              <h2 className="font-display text-[24px] font-bold text-ink mb-1">
                Scanning the Market...
              </h2>
        ...
        {phase === "results" && data && (
          ...
          >
            <ChannelDigestCard
              ...
              className="mb-6"
            />
            {data.dataReceipts?.length > 0 && <DataReceiptsStrip receipts={data.dataReceipts} />}
            ...
            <div className="flex items-start justify-between mb-10 gap-4 flex-wrap mt-6">
              <div>
                <button
                  onClick={reset}
                  className="text-[12px] font-semibold text-ink-soft hover:text-ink uppercase tracking-widest mb-3 flex items-center gap-1.5 transition"
                >
                  ← New Analysis
                </button>
                <h1 className="font-display text-[40px] font-bold tracking-tight text-ink leading-tight">
                  {data.nicheName}
                </h1>
              ...
              <div className="flex items-start gap-3">
                <div
                  className={`flex flex-col items-center justify-center rounded-[20px] border-2 px-8 py-4 shrink-0 ${GRADE_BG[data.overallGrade] || "bg-surface-2 border-hairline"}`}
                >
                  <div
                    className={`font-display text-[52px] font-black leading-none ${GRADE_COLORS[data.overallGrade] || "text-ink"}`}
                  >
                    {data.overallGrade}
                  </div>
                  <div className="text-[11px] font-bold uppercase tracking-widest text-ink-soft mt-1">
                    Viability Grade
                  </div>
                </div>
                <div className="flex flex-col gap-1.5 pt-2">
                  <button
                    onClick={() => onNicheFeedback("positive")}
                    className={`size-7 rounded-full flex items-center justify-center transition-all ${feedback === "positive" ? "bg-good/15 text-good" : "text-ink-soft hover:bg-surface-2"}`}
                    title="Accurate analysis"
                  >
                    <ThumbsUp size={14} />
                  </button>
                  <button
                    onClick={() => onNicheFeedback("negative")}
                    className={`size-7 rounded-full flex items-center justify-center transition-all ${feedback === "negative" ? "bg-critical/15 text-critical" : "text-ink-soft hover:bg-surface-2"}`}
              ...
              <div className="xl:col-span-8 space-y-6">
                  ...
                  {[
                    {
                      icon: Shield,
                      label: "Saturation",
                      value: `${data.metrics.saturation.label}`,
                      sub: `${data.metrics.saturation.score}/100`,
                      color:
                        data.metrics.saturation.score > 70 ? "text-red-500" : "text-emerald-500",
                    ...
                    },
                    {
                      icon: TrendingUp,
                      label: "Trend",
                      value: data.metrics.trendVelocity.direction,
                      sub: `${data.metrics.trendVelocity.score}/100`,
                      color:
                        data.metrics.trendVelocity.direction === "Rising"
                          ? "text-emerald-500"
                          : data.metrics.trendVelocity.direction === "Declining"
                            ? "text-red-500"
                            : "text-amber-500",
                      barColor:
                        data.metrics.trendVelocity.direction === "Rising"
                    ...
                    },
                    {
                      icon: Flame,
                      label: "Difficulty",
                      value: data.metrics.breakthroughDifficulty.label,
                      sub: `${data.metrics.breakthroughDifficulty.score}/100`,
                      color:
                        data.metrics.breakthroughDifficulty.score > 70
                          ? "text-red-500"
                          : "text-emerald-500",
                      barColor:
                    ...
                    <div
                      key={label}
                      className="bg-surface border border-hairline rounded-[20px] p-5 hover:shadow-md hover:border-ink/10 transition-all"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {[
                      ...
                      },
                      {
                        ...
                        insight: data.metrics.breakthroughDifficulty.insight,
                        score: data.metrics.breakthroughDifficulty.score,
                        barColor:
                          data.metrics.breakthroughDifficulty.score > 70 ? "bg-red-400" : "bg-good",
                      },
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-emerald-50 border border-emerald-100 rounded-[24px] p-6">
                    <h3 className="font-bold text-emerald-700 text-[14px] uppercase tracking-widest mb-4 flex items-center gap-2">
                    ...
                    <ul className="space-y-2.5">
                      {data.strengths.map((s: string, i: number) => (
                        <li
                          key={i}
                          className="flex items-start gap-2.5 text-[14px] text-emerald-800"
                        >
                          <span className="mt-0.5 w-5 h-5 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center text-[11px] font-bold text-emerald-600 shrink-0">
                            {i + 1}
                          </span>
                          {s}
                        </li>
                  ...
                  <div className="bg-red-50 border border-red-100 rounded-[24px] p-6">
                    <h3 className="font-bold text-red-700 text-[14px] uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Shield size={14} /> Warnings
                    </h3>
                    <ul className="space-y-2.5">
                      {data.warnings.map((w: string, i: number) => (
                        <li key={i} className="flex items-start gap-2.5 text-[14px] text-red-800">
                          <span className="mt-0.5 w-5 h-5 rounded-full bg-red-100 border border-red-200 flex items-center justify-center text-[11px] font-bold text-red-600 shrink-0">
                            !
                          </span>
                          {w}
                        </li>
              ...
              <div className="xl:col-span-4 space-y-6">
                ...
                <div className="bg-surface border border-hairline rounded-[24px] p-6 hover:border-ink/10 transition-all">
                  ...
                  <p className="text-[13px] text-ink-soft mb-5">
                    Don't go broad. These sub-niches give you a better shot at breaking through.
                  </p>
                  <div className="space-y-3">
                    {data.pivots.map((p: any, i: number) => (
                      <div
                        key={i}
                        className={`rounded-[16px] border p-4 ${GRADE_BG[p.grade] || "bg-surface-2 border-hairline"}`}
                      >
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <span className="font-bold text-[14px] text-ink leading-tight">
                            {p.subNiche}
                          </span>
                          <span
                            className={`text-[12px] font-black shrink-0 ${GRADE_COLORS[p.grade] || "text-ink"}`}
                          >
                            {p.grade}
                          </span>
                        </div>
                ...
                {/* Top Formats */}
                <div className="bg-surface border border-hairline rounded-[24px] p-6 hover:border-ink/10 transition-all">
                  <h2 className="font-semibold text-ink text-[16px] mb-4">
                  ...
                  <div className="space-y-2">
                    {data.topFormats.map((f: string, i: number) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 bg-surface-2 border border-hairline rounded-xl px-4 py-3"
                      >
